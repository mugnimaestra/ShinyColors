...
(function () {
	...
	*/
	...
	const googleApi = async keyword => {
	  ...
	  const res = await request$1("https://translate.google.com/translate_a/single?".concat(query), {
	    data: data,
	    method: 'POST',
	    headers: {
	      'accept': '*/*',
	  ...
	  try {
	    ...
	    if (config.myPage === 'multiple') {
	      Object.keys(homeDeckList).forEach(key => {
	        ...
	        tryDownload(str, "".concat(homeDeckList[key][0].speakerName, "-myPageComments.csv"));
	      });
